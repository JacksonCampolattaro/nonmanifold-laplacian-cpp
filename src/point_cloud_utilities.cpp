...
LocalTriangulationResult build_delaunay_triangulations(
    const std::vector<std::vector<Vector2>>& coords,
    const Neighbors_t& neigh
) {
    ...
    for (size_t iPt = 0; iPt < nPts; iPt++) {
        ...
        std::vector<Vector2> perturbPoints = coords[iPt]; {
            ...
            for (size_t iNeigh = 0; iNeigh < nNeigh; iNeigh++) {
                ...
                if (dist < lenScale * PERTURB_THRESH) {
                    // need to perturb
        ...
        std::vector<size_t> sortInds(nNeigh); {
            ...
            if (largestGap > (M_PI - ANGLE_COLLINEAR_THRESH)) {
                ...
                firstInd = std::distance(sortInds.begin(),
                                         std::find(sortInds.begin(), sortInds.end(), closestPointInd));
                hasBoundary = false;
            }
